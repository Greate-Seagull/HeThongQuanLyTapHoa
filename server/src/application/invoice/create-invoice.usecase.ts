...
export class CreateInvoiceUsecase {
	...
	async execute(input: any): Promise<CreateInvoiceOutput> {
		...
		const invoice = create(Invoice, {
			...
			invoiceDetails: serviceResult.items,
		});
		invoice.updateUserInfo(user, parsedInput.usedPoint);

		// Save transaction
		const save = await this.transactionManager.transaction(async (tx) => {
			const promisedUser = user ? this.userRepo.save(tx, user) : null;
			const promisedProducts = this.productRepo.save(tx, products);
			const promisedInvoice = this.invoiceRepo.add(tx, invoice);
